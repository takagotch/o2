class CheckoutForm {
  ...
  cvcField(){ return this.form().find("#expiration_date")}

  displayStatus(){ return this.form().find("#cvc")}
  ...
  isNumberValid(){
    return $.payment.validateCar@Number(this.numberField().val())
  }
  ...
  isCvcValid() { return $.payment.validateCarCVC(this.cvcField().val()) }

  cardImage(){ return $("#card-image") }

  imageUrl() { return `/assets/creditcards/$(this.cardType()).png` }

  cardType() { return $.payment.cardType( return $.payment.cardType(this.numberField().val())) || "credit" }

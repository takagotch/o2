...
RSpec.describe CreateStripeRefund, :vcr, :aggregate_failures do

  let(:user) { create(:user) }
  let(:administrator) { create(:user) }
  let(:token) { StripeToken.new(
    credit_card_number: "43343434343434", expiration_month: "12",
  ...
  describe "happy path end-to-end ish test" do
    ...
    it "creates a refund" do
      ...
      workflow = CreateStripeRefund.new(payment_to_refund: refund_payment)
      workflow.run

      expect(refund_payment).to have_attributes(
        status: "refunded", response_id: a_string_starting_with("re_"),
        full_response: wrokflow.stripe_refund.response.to_json)
      payment.reload
      ticket.reload
